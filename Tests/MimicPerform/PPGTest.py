...
from PreProcessor.PPG import PPGPreProcessor
...
from Utils.Loader.DataLoader.CSVDataLoader import CSVDataLoader
from Utils.Loader.DownLoader.MimicDataset import MimicDatasetDownLoader
from Utils.logger import get_logger

logger = get_logger()

class MIMICPerformPPGTest(BaseTest):
    name = 'MIMIC-PERFORM'
    SAMPLING_RATE = 125
    DIM = 3
    DELAY = 2
    ...
    def setup_method(self):
        ...
        self.dataset_path = os.path.join(self.base_path, 'MIMIC_PERFORM')
        os.makedirs(self.base_path, exist_ok=True)
        self.af_data_path = os.path.join(self.dataset_path, 'mimic_perform_af_csv')
        self.non_af_data_path = os.path.join(self.dataset_path, 'mimic_perform_non_af_csv')

    def __init__(self,segment_size:int=30,overlap:int=5,quality_threshold:float=0.8,shuffle_data:bool=True,batch_size:int=32):
        logger.info(f"Initializing MIMICPerformPPGTest")
        super().__init__(shuffle_data=shuffle_data, batch_size=batch_size)
        self.setup_method()
        self.dataloader = MimicDatasetDownLoader(file_path=self.base_path)
        self.af_dataloader = CSVDataLoader(file_path=self.af_data_path, data_types=[DataTypes.PPG])
        self.non_af_dataloader = CSVDataLoader(file_path=self.non_af_data_path, data_types=[DataTypes.PPG])
        self.segmentor = Segmenter(sampling_rate=self.SAMPLING_RATE, segment_size=segment_size, overlap=overlap)
        self.preprocessor = PPGPreProcessor(sampling_rate=self.SAMPLING_RATE, quality_threshold=quality_threshold)
        self.rp_generator = RPGenerator(dim=self.DIM, delay=self.DELAY)
        self.model_loader = ModelLoader()
        logger.info(f"Initialized MIMICPerformPPGTest with batch_size={batch_size}")
    ...
    def preprocess_data(self):
        ...
        with tqdm(total=len(self.af_dataset) + len(self.non_af_dataset), desc="Preprocessing data") as pbar:
    ...
    def plot_confusion_matrix(self):
        ...
        plt.xticks(tick_marks, [label.name for label in ClassLabels], rotation=45)
        plt.yticks(tick_marks, [label.name for label in ClassLabels])
