...
class MIMICPerformECGTest(BaseTest):
    name = 'MIMIC-PERFORM'
    SAMPLING_RATE = 125
    DIM = 3
    DELAY = 2
    ...
    def __init__(self,segment_size:int=30,overlap:int=5,quality_threshold:float=0.8,shuffle_data:bool=True,batch_size:int=32):
        logger.info(f"Initializing MIMICPerformECGTest")
        ...
        self.af_dataloader = CSVDataLoader(file_path=self.af_data_path, data_types=[DataTypes.ECG])
        self.non_af_dataloader = CSVDataLoader(file_path=self.non_af_data_path, data_types=[DataTypes.ECG])
        ...
        logger.info(f"Initialized MIMICPerformECGTest with batch_size={batch_size}")
