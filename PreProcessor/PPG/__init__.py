...
class PPGPreProcessor(BasePreProcessor):
    ...
    def __init__(self, sampling_rate, quality_threshold:float=0.9):
        ...
        logger.info(f"Initialized PPGPreProcessor with sampling_rate={sampling_rate}, quality_threshold={quality_threshold}")

    def preprocess(self, data:np.ndarray)->np.ndarray|None:
        try:
            cleaned_signal = nk.ppg_clean(data, sampling_rate=self.sampling_rate)
            quality = nk.ppg_quality(cleaned_signal, sampling_rate=self.sampling_rate)
            average_quality = np.mean(quality)
            if average_quality >= self.quality_threshold:
                info = nk.ppg_findpeaks(cleaned_signal, sampling_rate=self.sampling_rate)
                peak_distances = np.diff(info['PPG_Peaks'])
